...
import { TrendUp, TrendDown, Clock, CheckCircle, Warning, Sparkle } from '@phosphor-icons/react/ssr';
...
export default function PriceTrendAnalysis({ sourceCity, destinationCity, className = '', currentPrice, departureDate }: PriceTrendAnalysisProps) {
  ...
  const getTrendIcon = (direction: string) => {
    switch (direction) {
      case 'increasing': return <TrendUp className="w-5 h-5 text-neg-fg" />;
      case 'decreasing': return <TrendDown className="w-5 h-5 text-pos-fg" />;
      default: return <Clock className="w-5 h-5 text-caution-fg" />;
    }
  };
  ...
  const getRecommendationStyle = (_rec: string, action: string) => {
    switch (action) {
      case 'book_now':
      case 'book_soon': return 'bg-pos border-pos-fg/25 text-pos-fg';
      case 'wait': return 'bg-neg border-neg-fg/25 text-neg-fg';
      default: return 'bg-caution border-caution-fg/25 text-caution-fg';
    }
  };
  ...
  const getRecommendationIcon = (action: string) => {
    switch (action) {
      case 'book_now':
      case 'book_soon': return <CheckCircle className="w-5 h-5 text-pos-fg" />;
      case 'wait': return <Warning className="w-5 h-5 text-neg-fg" />;
      default: return <Clock className="w-5 h-5 text-caution-fg" />;
    }
  };
      ...
      <Card className={className}>
        <CardContent className="p-6">
          <div className="flex items-center justify-center space-x-2">
            ...
            <span className="text-sm text-ink-secondary">Loading price trends...</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {/* Mock Data Banner */}
      {isMockData && (
        <div className="flex items-center gap-2 px-4 py-2 bg-caution border border-caution-fg/25 rounded-lg text-sm text-caution-fg">
      ...
      {/* Coverage Notice */}
      {outOfCoverage && (
        <div className="flex items-center gap-2 px-4 py-2 bg-surface-sunken border border-line rounded-lg text-sm text-ink-secondary">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span>What the model recommends</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className={`p-4 rounded-lg border ${getRecommendationStyle(analysis.recommendation, analysis.action)}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-lg">
                  {analysis.action === 'book_now' ? 'Book now' :
                   analysis.action === 'book_soon' ? 'Book soon' :
                   analysis.action === 'wait' ? 'Hold' : 'Keep watching'}
                </span>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-3 bg-surface-sunken rounded-lg">
                <p className="text-xs text-ink-secondary mb-1">Current Price</p>
                <p className="font-bold text-lg">{formatCurrency(analysis.current_vs_predicted.current_price)}</p>
              </div>
              <div className="text-center p-3 bg-info rounded-lg">
                <p className="text-xs text-ink-secondary mb-1">ML Predicted Price</p>
                <p className="font-bold text-lg text-info-fg">{formatCurrency(analysis.current_vs_predicted.predicted_price)}</p>
              </div>
            </div>
            ...
            {analysis.best_booking_days && analysis.best_booking_days.length > 0 && (
              <div className="mt-3 p-3 bg-pos rounded-lg border border-pos-fg/25">
                <p className="text-xs font-semibold text-pos-fg mb-2">Best Days to Book</p>
            ...
            <div className="flex items-center justify-between gap-3 pt-1">
              ...
              <Button variant="outline" size="sm" disabled={alertSet} onClick={handleSetAlert}>
                {alertSet ? 'Alert set' : 'Set a price alert'}
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="text-center p-4 bg-info rounded-lg border border-info-fg/25">
                <p className="text-xs text-info-fg font-medium mb-1">AI Predicted</p>
                <p className="text-xl font-bold text-info-fg">{formatCurrency(analysis.current_vs_predicted.predicted_price)}</p>
                <p className="text-xs text-info-fg mt-1">Based on ML model</p>
              </div>
              <div className="text-center p-4 bg-pos rounded-lg border border-pos-fg/25">
                <p className="text-xs text-pos-fg font-medium mb-1">Real-time Price</p>
                <p className="text-xl font-bold text-pos-fg">{formatCurrency(currentPrice)}</p>
                <p className="text-xs text-pos-fg mt-1">Live market price</p>
              </div>
              <div className={`text-center p-4 rounded-lg border ${
                currentPrice < analysis.current_vs_predicted.predicted_price
                  ? 'bg-pos border-pos-fg/25' : 'bg-neg border-neg-fg/25'
              }`}>
                <p className={`text-xs font-medium mb-1 ${
                  currentPrice < analysis.current_vs_predicted.predicted_price ? 'text-pos-fg' : 'text-neg-fg'
                }`}>
            ...
            <div className="p-3 bg-surface-sunken rounded-lg">
              ...
              <div className="relative h-4 bg-gradient-to-r from-pos via-caution to-neg rounded-full overflow-hidden">
                {/* Predicted price marker */}
                <div
                  className="absolute top-0 h-full w-1 bg-info-fg z-10"
                  style={{
                ...
                <div
                  className="absolute top-0 h-full w-1 bg-pos-fg z-10"
                  style={{
              ...
              <div className="flex justify-center gap-6 mt-2 text-xs">
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-info-fg rounded-sm inline-block" /> AI Predicted</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-pos-fg rounded-sm inline-block" /> Real-time</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {trends.length > 0 ? (
            <div className="space-y-4">
              <div className="h-32 bg-surface-sunken rounded-lg overflow-x-auto p-4">
                  ...
                  {trends.map((trend, index) => {
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center space-y-1">
                        <div
                          className={`w-3 rounded-t ${trend.is_weekend ? 'bg-caution-fg' : 'bg-brand'}`}
                          style={{ height: `${height}px` }}
                          title={`${trend.day_of_week || ''}: ${formatCurrency(trend.predicted_price)}${trend.is_weekend ? ' (weekend)' : ''}`}
                        />
                        <span className="text-xs text-ink-tertiary rotate-45 origin-bottom-left">
              ...
              {/* Legend */}
              <div className="flex justify-center gap-4 text-xs text-ink-tertiary">
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-info-fg rounded-sm inline-block" /> Weekday</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-caution-fg rounded-sm inline-block" /> Weekend</span>
              </div>

              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="p-2 bg-pos rounded">
                  <p className="font-semibold text-pos-fg">Lowest Predicted</p>
                  <p className="text-pos-fg">{formatCurrency(Math.min(...trends.map(t => t.predicted_price)))}</p>
                </div>
                <div className="p-2 bg-info rounded">
                  <p className="font-semibold text-info-fg">Average</p>
                  <p className="text-info-fg">{formatCurrency(trends.reduce((sum, t) => sum + t.predicted_price, 0) / trends.length)}</p>
                </div>
                <div className="p-2 bg-neg rounded">
                  <p className="font-semibold text-neg-fg">Highest Predicted</p>
                  <p className="text-neg-fg">{formatCurrency(Math.max(...trends.map(t => t.predicted_price)))}</p>
                </div>
              </div>
            </div>
          ) : (
